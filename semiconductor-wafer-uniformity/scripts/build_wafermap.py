...
cx,cy=W_PT/2, 500
scale=2.55   # mm -> pt (75mm -> ~191pt radius) for the wafer body/edge
Rpx=W.WAFER_RADIUS_MM*scale
# node placement scale: pull the outermost ring (73 mm) to ~84% of the wafer
# radius so every ring, including the outer one, sits clear of the edge with
# uniform proportional spacing. Layout is not-to-scale by design.
OUTER_FRAC=0.84
outer_ring_r=max(r for (r,nn,off) in W.RINGS)   # 73 mm
nscale=OUTER_FRAC*Rpx/outer_ring_r
# wafer body
c.setStrokeColor(INK); c.setLineWidth(1.6); c.setFillColor(HexColor("#F4F7F9"))
c.circle(cx,cy,Rpx,stroke=1,fill=1)
# edge exclusion ring (dashed red), drawn just inside the outer ring of nodes
c.setDash(4,3); c.setStrokeColor(EXCL); c.setLineWidth(1.2)
c.circle(cx,cy,W.EXCL_RADIUS*nscale,stroke=1,fill=0); c.setDash()
# notch at bottom (on the wafer body edge)
...
# ring guide circles (faint) at node scale, so nodes sit exactly on them
c.setStrokeColor(HexColor("#DDDDDD")); c.setLineWidth(0.7)
for (r,nn,off) in W.RINGS[1:]:
    c.circle(cx,cy,r*nscale,stroke=1,fill=0)

# sites (placed on the node scale; every ring's nodes land on its guide circle)
for s in W.SITES:
    x=cx+s["x"]*nscale; y=cy+s["y"]*nscale
    excl=s["excluded"]
